function AM201410plotSFINCSRubyMultiSpeciesConvergenceScan()
...
for iFile = 1:size(files,1)
    ...
    % AM
    %if RHSMode2s > 0
    %TransportArray(runNum,((1:3)-1)*3+1) = h5read(filename,[location,'ArrayFirstSpeciesParticleFluxCoefficients']);
    TransportArray(runNum,(1:3)) = h5read(filename,[location,'ArrayFirstSpeciesParticleFluxCoefficients']);
    %end
...
if numel(values)>1
    ...
    TransportCoefficients{end+1} = TransportArray(runIndices, :);
end
...
TransportCoefficientsMaxs=ones(numTransportCoefficients,1)*(-Inf);
TransportCoefficientsMins=ones(numTransportCoefficients,1)*Inf;

for iParameter = 1:numParameters
    maxs = max([maxs, quantities{iParameter}'],[],2);
    mins = min([mins, quantities{iParameter}'],[],2);
    
    TransportCoefficientsMaxs = max([TransportCoefficientsMaxs, TransportCoefficients{iParameter}'],[],2);
    TransportCoefficientsMins = min([TransportCoefficientsMins, TransportCoefficients{iParameter}'],[],2);    
end
...
for iQuantity = 1:3
    if TransportCoefficientsMaxs(iQuantity) <= TransportCoefficientsMins(iQuantity)
        TransportCoefficientsMaxs(iQuantity) = TransportCoefficientsMins(iQuantity)+1;
    end
    for iParameter = 1:numParameters
        subplot(3, numCols, iParameter  + (iQuantity - 1)*numParameters)
        plot(abscissae{iParameter}, TransportCoefficients{iParameter}(:,iQuantity)', linespecs{iQuantity})
        hold on
        plot([convergeds{iParameter}, convergeds{iParameter}], [TransportCoefficientsMins(iQuantity),TransportCoefficientsMaxs(iQuantity)],'k')
        ylim([TransportCoefficientsMins(iQuantity), TransportCoefficientsMaxs(iQuantity)])
        xlabel(parametersToVary{iParameter})
        ylabel(yAxesLabels{plotRows(iQuantity)})
        if iQuantity == 1
            ylabel('L_{11}^{zz}')
