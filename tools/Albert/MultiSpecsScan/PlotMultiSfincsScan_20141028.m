function PlotMultiSfincsScan_20141028()
...
PrintToFile = 1; %If PrintToFile = 1 the plotted data is also printed into .dat files
outputFilePrefix = 'W7X_StandardConfig_ra0p88_Zeff1p05_';
...
yAxesLabels = {'L_{11}^{zz}', 'L_{11}^{zi}', 'L_{12}^{zz} + L_{12}^{zi}'};
%xAxesLabels = {'n_{z} (= C \cdot n_{i})', 'd \Phi / d \psi'};
xAxesLabels = {'n_{z} (= C \cdot n_{i})'};
...
for j=1:length(QuantitiesToPlotArray)
    for k=1:length(QuantitiesOnXaxisArray)
        for l=1:length(PlotYarray{j,k,1})
            ...
            %Print to file
            if PrintToFile == 1
                dlmwrite(strcat(outputFilePrefix, yAxesLabel, '.dat'), cat(2, XarraySorted.', YarraySorted.')); 
            end
