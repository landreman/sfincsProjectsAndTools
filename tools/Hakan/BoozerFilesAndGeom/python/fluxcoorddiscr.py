...
# % Discr = Ham, Booz, or Pest. The Fourier representation of B, R and Z in the respective 
...
class fluxcoorddiscr:
  ...
  def __init__(self,Geom,rind,Ntheta,Nzeta,name='Boozer'):
      ...
      if not((name=='Boozer') or (name=='Hamada') or (name=='Pest') or (name=='Cylindrical')):
                    sys.exit('The coordinate name must be one of '+
                             'Boozer, Hamada, Pest or Cylindrical!')
      ...
      Bmn=mnmat(Geom,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='B')
      ...
      umn=mnmat(h,Nperiods=NPeriods).calcu(G,I,iota) 
      u=umn.ifft()

      Dzetaphimn=mnmat(1-h*FSAB2,Nperiods=NPeriods).invJacBdotgrad(iota)
      ...
      #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      self.Nperiods=Geom.Nperiods
      self.iota=Geom.iota[rind]
      self.G=Geom.Bphi[rind]
      self.I=Geom.Btheta[rind]
      ...
      self.dpsidrGraz=self.FSAsqrtgpsipsi*np.sign(Geom.psi_a)
      self.dsdrGraz  =self.FSAsqrtgpsipsi/abs(Geom.psi_a)
      ...
      if name=='Hamada' or name=='Pest':
        ...
        Nleftadd=np.int(np.ceil(max(Booz_newtor[:,0])/(2*np.pi/self.Nperiods)))
        Nrightadd=np.int(np.ceil(1-min(Booz_newtor[:,-1])/(2*np.pi/self.Nperiods)))
        
        tmp=np.zeros((self.Npol*3,self.Ntor*(Nleftadd+1+Nrightadd)))
        BooznewtorBig      =np.nan*np.zeros((self.Npol*3,self.Ntor*(Nleftadd+1+Nrightadd)))
        BooznewpolBig      =np.nan*np.zeros((self.Npol*3,self.Ntor*(Nleftadd+1+Nrightadd)))
        BoozDzetanewtorBig =np.nan*np.zeros((self.Npol*3,self.Ntor*(Nleftadd+1+Nrightadd)))

        for addi in range(Nleftadd+1+Nrightadd):
          addci=addi-Nleftadd #is =0 for the original
          toaddtor=2*np.pi/self.Nperiods*addci
          BooznewtorBig[          0:1*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_phi+toaddtor
          BooznewtorBig[  self.Npol:2*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_phi+toaddtor
          BooznewtorBig[2*self.Npol:3*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_phi+toaddtor
          BooznewpolBig[          0:1*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_vthet-2*np.pi
          BooznewpolBig[  self.Npol:2*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_vthet
          BooznewpolBig[2*self.Npol:3*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=Booz_vthet+2*np.pi
          BoozDzetanewtorBig[          0:1*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=BoozDzetanewtor
          BoozDzetanewtorBig[  self.Npol:2*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=BoozDzetanewtor
          BoozDzetanewtorBig[2*self.Npol:3*self.Npol,addi*self.Ntor:(addi+1)*self.Ntor]=BoozDzetanewtor

        #print newpol.shape
        #print newtor.shape
        #print BooznewpolBig.flatten().shape
        #print BooznewtorBig.flatten().shape
        #print BoozDzetanewtorBig.flatten().shape
        #tmp=np.concatenate((np.expand_dims(BooznewtorBig.flatten(),axis=0).T,
        #                    np.expand_dims(BooznewpolBig.flatten(),axis=0).T),axis=1)
        #print tmp.shape
        
        New_Dzetanewtor=interpolate.griddata(
             np.concatenate((np.expand_dims(BooznewtorBig.flatten(),axis=0).T,
                             np.expand_dims(BooznewpolBig.flatten(),axis=0).T),axis=1),
                            BoozDzetanewtorBig.flatten(),(newtor,newpol))
        ...
        self.Bmn=mnmat(self.B,Nperiods=self.Nperiods)
  ...
  def plot(self,toshow,cmap=None):
        if cmap==None:
           #cmap='coolwarm'
           #cmap='RdBu'
           cmap='jet'
  ...
  def plot3d(self,toshow,torstride=1, polstride=1,cmap=None):
