...
class fluxcoorddiscr:
  ...
  def __init__(self,Geom,rind,Ntheta,Nzeta,name='Boozer'):
        ...
        if isinstance(Geom,bcgeom):
            ...
            halfgrid_dpds   = np.diff(wout.presf[skip-1:])./np.diff(fullgrid_s[skip-1:])
            psi_a=wout.phi[wout.ns-1]*signchange/2/np.pi
            ...
            uw_Bmn=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='B')
            uw_Rmn=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='R')
            uw_Zmn=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='Z')
            uw_B_umntilde=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='B_u').remove00()
            uw_B_wmntilde=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='B_w').remove00()
            uw_lmn=mnmat(wout,Ntheta=Ntheta,Nzeta=Nzeta,rind=rind,quantity='lambda')
      
            alphamn1=B_umntilde.invgrad(uw_B_wmntilde,method=1)
            alphamn2=B_umntilde.invgrad(uw_B_wmntilde,method=2)
            ...
            #u,w discretisation
            Dtheta=2*np.pi/Ntheta
            Dzeta=2*np.pi/Nzeta/self.Nperiods
            uniform_pol,uniform_tor = np.mgrid[0.0:2.0*np.pi-Dtheta:1j*Ntheta,0.0:2.0*np.pi/self.Nperiods-Dzeta:1j*Nzeta]
            uw_u = uniform_pol
            uw_w = uniform_tor
            ...
            Booz_Dthetau =griddatacyclic(uw_theta,uw_zeta,uw_Dthetau,self.Nperiods)
            Booz_Dzetaw  =griddatacyclic(uw_theta,uw_zeta,uw_Dzetaw, self.Nperiods)
            Dzetacylphi=Booz_Dzetaw
            #Pest_Dpthetau=griddatacyclic(uw_ptheta,uw_pzeta,uw_Dpthetau,self.Nperiods)
      
            Booz_u=Booz_theta-Booz_Dthetau        
            Booz_w=Booz_zeta-Booz_Dzetaw
            ...
            B=interp2_cyclic(uw_u,uw_w,uw_B,Booz_u,Booz_w,self.Nperiods)
            R=interp2_cyclic(uw_u,uw_w,uw_R,Booz_u,Booz_w,self.Nperiods)
            Z=interp2_cyclic(uw_u,uw_w,uw_Z,Booz_u,Booz_w,self.Nperiods)
      
            Bmn=mnmat(B,Nperiods=self.Nperiods)
            Rmn=mnmat(R,Nperiods=self.Nperiods)
            Zmn=mnmat(Z,Nperiods=self.Nperiods)
            Dzetacylphimn=mnmat(Booz_Dzetacylphi,Nperiods=self.Nperiods)
