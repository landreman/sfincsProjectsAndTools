...
for rind=1:length(dkdata.Nruns)
  ...
  dkdata.fmn{rind}=zeros(size(dkdata.d11{rind}));
  dkdata.lalpha{rind}=zeros(size(dkdata.d11{rind}));
  ...
  for runind=1:dkdata.Nruns(rind)
    ...
    dkdata.line2{rind}{runind}=sprintf(' >3       %6d %6d %13.5E %13.5E %13.5E',...
                                       dkdata.fmn{rind}(runind),...
                                       dkdata.lalpha{rind}(runind),...
                                       dkdata.d11e{rind}(runind),...
                                       dkdata.d31e{rind}(runind),...
                                       dkdata.d33e{rind}(runind));
  end
end
...
Geomdat.Nperiods=Geom.Nperiods;
Geomdat.StelSym=Geom.StelSym;
...
Geomdat.nmodes=Geom.nmodes(Grind);
...
dk_fid=fopen([strconf,'.sfincs.dk'],'w');
