...
for rind=1:dkdata.Nradii
  ...
  R=Geomdat.R00(rind);
  epsilon=Geomdat.rnorm(rind)*Geomdat.minorradius/R;
  ...
  disp(['Radius ',num2str(rind),'/',num2str(dkdata.Nradii),...
        ': r=',num2str(Geomdat.minorradius*Geomdat.rnorm(rind)),...
  ...
  while not(acceptedfit)
    ...
    xlabel(['(\nu/v)^{',num2str(exp_fit),'}'])
    ylabel('g_{11} = (\nu/v) * |\Gamma_{11}|')
    title(['r=',num2str(100*Geomdat.minorradius*Geomdat.rnorm(rind)),...
  ...
  if not(makeafit)
    ...
    while not(acceptedfit)
      ...
      title(['r=',num2str(Geomdat.minorradius*Geomdat.rnorm(rind)),' m,',...
